...
export class KafkaMessageBrokerAdapter implements MessageBrokerPort {
  public constructor(
    private readonly kafkaMessageBusHandler: KafkaMessageBusHandler,
  ...
  public async publishMessage(topic: string, payload: string): Promise<void> {
    ...
    await this.kafkaMessageBusHandler.handle(kafkaPublishMessageOperation, {
  ...
  public async subscribeTo(topic: string): Promise<void> {
    ...
    await this.kafkaMessageBusHandler.handle(kafkaSubscribeOperation, {
